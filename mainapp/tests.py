...
class TestNewsPage(TestCase):
    fixtures = (
        "LMS/mainapp/fixtures/001_news.json",
        "LMS/authapp/fixtures/001_user_admin.json",
    )
    ...
    def setUp(self):
        ...
        self.client_with_auth.force_login(self.user_admin, backend="authapp.auth.EmailAuthBackend")
...
class TestCoursesWithMock(TestCase):
    fixtures = (
        "LMS/authapp/fixtures/001_user_admin.json",
        "LMS/mainapp/fixtures/002_courses.json",
        "LMS/mainapp/fixtures/003_lessons.json",
        "LMS/mainapp/fixtures/004_teachers.json",
    )

    def test_page_open_detail(self):
        course_obj = mainapp_models.Courses.objects.get(pk=1)
        path = reverse("mainapp:courses_detail", args=[course_obj.pk])
        with open("LMS/mainapp/fixtures/005_feedback_list_1.bin", "rb") as inpf, mock.patch(
...
class TestTaskMailSend(TestCase):
    fixtures = ("LMS/authapp/fixtures/001_user_admin.json",)
...
class TestNewsSelenium(StaticLiveServerTestCase):
    ...
    def setUp(self):
        ...
        options.add_argument("/home/roman/snap/firefox/common/.mozilla/firefox/b6w8wpbd.selenium")

        super().setUp()
        self.selenium = WebDriver(executable_path=settings.SELENIUM_DRIVER_PATH_FF, options=options)
    ...
    def test_pick_color(self):
        ...
        try:
            ...
            with open("LMS/var/screenshots/001_navbar_el_scrnsht.png", "wb") as outf:
